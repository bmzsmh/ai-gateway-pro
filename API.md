...
Model Group 不再对所有 HTTP 4xx 自动 fallback。
...
- `401/403`
- `5xx`
- 网络错误/上游超时（Gateway 以 `502` 表示）

以下错误通常直接返回给客户端，不复制请求到其他模型：
...
单个 Group 请求默认最多尝试 6 个上游成员，避免模型和 Key 数量造成请求放大。
...
请在排查 Hermes Agent 问题时保留这个 ID。

### OpenCode Mirror

公共 OpenCode mirror fallback 默认关闭。只有同时明确开启：

```text
OPENCODE_MIRROR_FALLBACK=true
OPENCODE_MIRRORS_URL=...
```

才会把官方 OpenCode 请求失败后的可重试错误转发到镜像。
