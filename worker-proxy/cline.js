 ...
 *  - API_KEY                (必需) Worker 访问密钥；未配置时 Worker 拒绝所有业务请求
 *
 * 用法（OpenAI 兼容）：
...
export default {
  async fetch(request, env) {
    ...
    // 全局鉴权：所有业务端点都需要 API_KEY；未配置时 fail closed。
    const configuredKey = env.API_KEY;
    if (!configuredKey) {
      return jsonResponse({ error: { message: "Worker API_KEY 未配置", type: "configuration_error" } }, 503);
    }

    // 健康诊断端点也必须鉴权，避免泄露账号池/配置状态。
