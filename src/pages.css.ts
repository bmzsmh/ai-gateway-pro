...
.gi .cii > div { min-width: 0; }
.gi .ci-ico { width: 2.5rem; height: 2.5rem; flex: 0 0 auto; display: flex; align-items: center; justify-content: center; border-radius: var(--radius-control); background: var(--color-paper-2); color: var(--color-accent); }
.gi .gmute { color: var(--color-muted); font-size: var(--text-xs); }
.gi .ci-date { color: var(--color-muted); font-size: var(--text-xs); white-space: nowrap; }
.members-wrap { margin-block-start: var(--space-sm); padding-block-start: var(--space-sm); border-block-start: .0625rem solid var(--color-rule); }
.gmembers { display: flex; flex-wrap: wrap; gap: var(--space-2xs); }
.tag-multimodal { display: inline-flex; align-items: center; gap: .25rem; padding: .0625rem .5rem; border-radius: 999px; background: var(--color-accent); color: #fff; font-size: var(--text-xs); font-weight: 600; }
.chip { display: inline-flex; align-items: center; gap: .25rem; padding: .1875rem .5rem; border: .0625rem solid var(--color-rule-2); border-radius: 999px; background: var(--color-paper-2); font-size: var(--text-xs); }
.chip-model { font-family: var(--font-mono, monospace); }
.chip-avail { cursor: pointer; transition: border-color var(--dur-fast) var(--ease-out), color var(--dur-fast) var(--ease-out); }
.chip-avail:hover { border-color: var(--color-accent); color: var(--color-focus); }
.chip-x { display: inline-flex; align-items: center; justify-content: center; width: 1rem; height: 1rem; border: 0; border-radius: 50%; background: transparent; color: var(--color-muted); cursor: pointer; }
.chip-x:hover { background: var(--color-danger, #dc3545); color: #fff; }
...
/* ===== 监控面板（telemetry：请求日志 / 活跃模型 / 今日用量统计） ===== */
:root {
  --color-warn: oklch(65% 0.150 75);
  --color-warn-soft: oklch(96% 0.030 75);
  --color-warn-ink: oklch(40% 0.110 75);
}
.mon-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(15.5rem, 1fr)); gap: var(--space-sm); margin-bottom: var(--space-md); }
.mon-active-card { border: .0625rem solid var(--color-paper-3); border-radius: var(--radius-lg); padding: var(--space-sm); background: var(--color-paper); }
.mon-active-card h4 { margin: 0 0 var(--space-2xs); font-size: var(--text-sm); font-weight: 600; }
.mon-active-card .mon-active-body { font-size: var(--text-xs); color: var(--color-muted); font-family: var(--font-mono); }
.mon-active-card .mon-active-empty { color: var(--color-muted); font-style: italic; }
.mon-controls { display: flex; gap: var(--space-xs); align-items: center; flex-wrap: wrap; margin-bottom: var(--space-sm); }
.mon-usage-bar { position: relative; height: .5rem; border-radius: var(--radius-round); background: var(--color-paper-3); overflow: hidden; margin-top: var(--space-2xs); }
.mon-usage-bar > span { display: block; height: 100%; background: var(--color-accent); border-radius: var(--radius-round); transition: width var(--dur-fast) ease; }
.mon-usage-bar.is-warn > span { background: var(--color-warn); }
.mon-usage-bar.is-danger > span { background: var(--color-danger); }
.mon-usage-meta { display: flex; justify-content: space-between; font-size: var(--text-xs); color: var(--color-muted); margin-top: var(--space-3xs, .125rem); }
.mon-log-table { width: 100%; border-collapse: collapse; font-size: var(--text-xs); font-family: var(--font-mono); }
.mon-log-table th, .mon-log-table td { padding: .375rem .5rem; text-align: left; border-bottom: .0625rem solid var(--color-paper-3); white-space: nowrap; }
.mon-log-table th { color: var(--color-muted); font-weight: 600; font-family: var(--font-sans); }
.mon-status-chip { display: inline-flex; align-items: center; justify-content: center; min-width: 2.75rem; padding: .0625rem .375rem; border-radius: var(--radius-round); font-weight: 600; }
.mon-status-2xx { background: var(--color-success-soft); color: var(--color-success-ink); }
.mon-status-429 { background: var(--color-warn-soft); color: var(--color-warn-ink); }
.mon-status-4xx { background: var(--color-danger-soft); color: var(--color-danger-ink); }
.mon-status-5xx { background: var(--color-danger-soft); color: var(--color-danger-ink); }
.mon-log-empty { padding: var(--space-md); text-align: center; color: var(--color-muted); font-size: var(--text-sm); }
`
