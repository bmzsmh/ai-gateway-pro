...
export function resolveOpenCodeUrls(env: Env): string[] {
  // 公共镜像属于额外的数据信任边界，必须显式开启。
  if (env.OPENCODE_MIRROR_FALLBACK !== 'true') return []
...
function isRetryableMirrorStatus(status: number): boolean {
  return status === 408 || status === 425 || status === 429 || status === 401 || status === 403 || status >= 500
}
...
export async function proxyOpenCodeRequest(options: OpenCodeRequestOptions): Promise<Response> {
  ...
  for (const entry of enabledKeys) {
    try {
      ...
      if (!isRetryableMirrorStatus(response.status)) break
    } catch (error) {
